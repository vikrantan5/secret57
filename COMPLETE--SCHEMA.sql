-- =====================================================
-- SERVICEHUB COMPLETE DATABASE SCHEMA (FIXED)
...
CREATE INDEX IF NOT EXISTS bookings_seller_id_idx ON public.bookings(seller_id);
...
CREATE INDEX IF NOT EXISTS custom_orders_customer_id_idx ON public.custom_orders(customer_id);
CREATE INDEX IF NOT EXISTS custom_orders_seller_id_idx ON public.custom_orders(seller_id);
...
CREATE INDEX IF NOT EXISTS platform_stats_date_idx ON public.platform_stats(date);
...
-- Sellers Policies
DROP POLICY IF EXISTS "Sellers can read own data" ON public.sellers;
CREATE POLICY "Sellers can read own data" ON public.sellers FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Sellers can insert own data" ON public.sellers;
CREATE POLICY "Sellers can insert own data" ON public.sellers FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Sellers can update own data" ON public.sellers;
...
DROP VIEW IF EXISTS seller_revenue;
...
-- All tables, indexes, policies, and views are ready.
